...
var formats = {
    javascript: {
        ...
        format: function (locales, options) {
            var formatString = "_0_";
            var module = "angular.module('" + options.module + "').run(['gettextCatalog', function (gettextCatalog) {\n/* jshint -W100 */\n" + locales.join('') + "/* jshint +W100 */\n}]);";
            if (options.requirejs) {
                formatString = "define(['angular', '" + options.modulePath + "'],function(angular){\n_0_\n});";
            }

            return formatString.replace(/_0_/, module);
        }
    },
    json: {
        addLocale: function (locale, strings) {
