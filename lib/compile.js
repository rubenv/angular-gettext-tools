    ...
    javascript: {
        ...
        format: function (locales, options) {
            ...
            if (options.requirejs) {
                var modulePath = (options.requirejsModulePath) ? options.requirejsModulePath : '';
                var deps = (options.requirejsDependencies) ? JSON.stringify(options.requirejsDependencies).replace(/"/gi, '\'') : '';
                return 'define(\'' + modulePath + '\',' + deps + ', function () {\nreturn ' + module + '\n});';
            }
