...
var Extractor = (function () {
    ...
    Extractor.prototype.extractJs = function (filename, src) {
        ...
        walkJs(syntax, function (node, parentComment) {
            ...
            if (str || singular) {
                ...
                if (str) {
                    extractor.addString(filename, str, "", extractedComments);
                }
                else if (singular) {
                    extractor.addString(filename, singular, plural, extractedComments);
                }
            }
        });
    };
    ...
    Extractor.prototype.extractHtml = function (filename, src) {
        ...
        $('*').each(function (index, n) {
            var node, plural, str, matches, extractedComment;
            node = $(n);
            ...
            for (var attr in node.attr()) {
                if (attr === 'translate') {
                    ...
                    extractor.addString(filename, str, plural, extractedComment);
                }
