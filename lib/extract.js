...
var mkPluralAttrRegex = function (startDelim, endDelim) {
    var start = startDelim.replace(escapeRegex, '\\$&');
    var end = endDelim.replace(escapeRegex, '\\$&');
    ...
    return new RegExp(start + '\\s*(\'|"|&quot;)(.*?)\\1\\s*\\|\\s*translatePlural:.*:\\1(.*?)\\1\\s*(' + end + '|\\|)', 'g');
};

var noDelimRegex = mkAttrRegex('', '');
var noDelimPluralRegex = mkPluralAttrRegex('', '');
...
var Extractor = (function () {
    function Extractor(options) {
        ...
        this.pluralAttrRegex = mkPluralAttrRegex(this.options.startDelim, this.options.endDelim);
    }
    ...
    Extractor.mkPluralAttrRegex = mkPluralAttrRegex;
    ...
    Extractor.prototype.extractHtml = function (filename, src) {
        ...
        $('*').each(function (index, n) {
            ...
            for (var attr in node.attr()) {
                ...
                } else if (matches = noDelimPluralRegex.exec(node.attr(attr))) {
                    str = matches[2].replace(/\\\'/g, '\'');
                    plural = matches[3].replace(/\\\'/g, '\'');
        ...
        while (matches = this.pluralAttrRegex.exec(src)) {
            str = matches[2].replace(/\\\'/g, '\'');
            var plural = matches[3].replace(/\\\'/g, '\'');
