...
var Extractor = (function () {
    ...
    Extractor.prototype.addString = function (reference, string, plural, extractedComment, context) {
        ...
        if (extractedComment && extractedComment.length > 0) {
            var i = search(item.extractedComments, extractedComment, function (a, b) {
                return a.localeCompare(b);
            });
            if (i < 0) { // don't add duplicate comments
                // when not found, binary-search returns -(index_where_it_should_be + 1)
                item.extractedComments.splice(Math.abs(i + 1), 0, extractedComment);
            }
        }
    };
