...
var Extractor = (function () {
    ...
    Extractor.prototype.addString = function (file, string, plural, extractedComment, context) {
        ...
        if (context === undefined) {
            // Don't convert '' to null. From the gettext docs:
        ...
        var findItem = function () {
            var item;
            var existingStrings = self.strings[string];
            if (!existingStrings) {
                self.strings[string] = new Po.Item();
                return self.strings[string];
            }

            var contextsEqual = function (c1, c2) {
                if (c1 === null || c2 === undefined) {
                    // undefined and null are the same to us
                    return c2 === null || c2 === undefined;
                }
                return c1 === c2;
            };

            if (Array.isArray(existingStrings)) {
                item = _.find(existingStrings, function (item) {
                    return contextsEqual(context, item.msgctxt);
                });
                if (!item) {
                    item = new Po.Item();
                    existingStrings.push(item);
                }

                return item;
            } else {
                if (contextsEqual(context, existingStrings.msgctxt)) {
                    return existingStrings;
                } else {
                    self.strings[string] = [existingStrings];
                    item = new Po.Item();
                    self.strings[string].push(item);
                    return item;
                }
            }
        };
